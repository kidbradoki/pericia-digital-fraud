...
H = """
<!DOCTYPE html><html lang="pt-br"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no"><title>GHOST v8.7</title><style>
...
</style></head><body><canvas id="m"></canvas><div class="app"><h2>CENTRAL GHOST v8.7</h2>
...
<div class="mod"><h3>MOD 04: PERÍCIA</h3><div class="g3">
<button onclick="t('fe')" class="btn">ARQUIVO</button>
<button onclick="document.getElementById('fbtn').click()" class="btn">ANALISAR</button>
<a href="/" class="btn">LIMPAR</a></div>
<div id="fe" class="eng" style="display:{% if ma %}block{% else %}none{% endif %};">
<form id="periciaForm" action="/a" method="post" enctype="multipart/form-data">
<input type="file" name="file" id="fileInput" required>
<button id="fbtn" class="btn" style="width:100%">PROCESSAR ALVO</button></form>
...
def a():
    ...
    if not f: return render_template_string(H, ma="⚠️ SELECIONE UM ARQUIVO.")
    ...
    try:
        if ext in ['jpg', 'jpeg', 'png']:
            ...
            if any(x in d.upper() for x in ["CANVA", "EDITOR", "ILLUSTRATOR"]):
                alert = "<span class='warn'>[!] PDF EDITADO</span>"
            res = f"{alert}📄 <b>INFO:</b> {d}<br>📊 <b>PÁGINAS:</b> {len(pdf.pages)}"
        else: res = "⚠️ FORMATO NÃO SUPORTADO."
    except Exception as e:
        res = f"❌ ERRO TÉCNICO: Verifique se o Tesseract está instalado no packages.txt"
