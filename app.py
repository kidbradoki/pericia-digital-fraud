        ...
        <div class="box-full">
            ...
            <div class="status-badge {{ 'fraude' if r.status == 'A' else 'real' }}">
                ...
                <div class="dados-container">
                    <strong>CONTEÚDO EXTRAÍDO:</strong><br><br>
                    {{ r.dados }}
                </div>
            </div>
            {% endif %}
...
def motor_analise(caminho):
    ...
    try:
        ...
        return {"status": "A", "titulo": "⚠️ ERRO", "mensagem": "Falha crítica na leitura.", "dados": "N/A"}

    if not texto.strip():
        return {"status": "A", "titulo": "⚠️ PDF SEM TEXTO", "mensagem": "O arquivo parece ser apenas uma imagem.", "dados": "Vazio"}

    # Limpeza básica para exibição
    ...
    if "PIX" in texto or "TRANSACAO" in texto:
        if "AGENDAMENTO" in texto and "COMPROVANTE" in texto:
            return {"status": "A", "titulo": "⚠️ GOLPE: PIX AGENDADO", "mensagem": "Comprovante de agendamento mascarado.", "dados": resumo}
        return {"status": "O", "titulo": "✅ PIX IDENTIFICADO", "mensagem": "Padrão de transferência bancária.", "dados": resumo}
    ...
    if any(x in texto for x in ["REGISTRO GERAL", "CNH", "CPF", "IDENTIDADE", "CERTIDAO", "TRIBUNAL"]):
        return {"status": "O", "titulo": "✅ DOCUMENTO PESSOAL", "mensagem": "Dados de identificação civil/jurídica.", "dados": resumo}

    return {"status": "A", "titulo": "⚠️ DOC DESCONHECIDO", "mensagem": "Conteúdo extraído para análise manual.", "dados": resumo}
