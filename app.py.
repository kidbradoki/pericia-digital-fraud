...
# Lógica de Região Fiscal (Exemplo)
...
def executar():
    ...
    if acao == 'pericia':
        ...
        return jsonify({
            "status": "sucesso",
            "resultado": f"Região Fiscal: {regiao}",
            "alertas": "Instituição com alto índice de reclamações detectada." if "Celcoin" in valor else "Nenhum alerta crítico."
        })
    
    elif acao == 'osint':
        return jsonify({"status": "sucesso", "resultado": f"Rastreando pegada digital para: {valor}..."})
